...
use super::timeline::{ActivityTimeline, TimePeriod, TimelineConfig};
...
impl TimelineCache {
    ...
    fn cache_file_path(&self, config: &TimelineConfig) -> PathBuf {
        ...
        self.cache_dir
            .join(format!("timeline_{:x}.json", config_hash))
    }
    ...
    pub fn hash_conversation_directory(
        &self,
        conversations_dir: &Path,
    ) -> Result<u64, ClaudeToolsError> {
        let mut hasher = DefaultHasher::new();
    ...
    pub fn load_timeline(
        ...
        config: &TimelineConfig,
        conversations_dir: &Path,
    ) -> Result<Option<ActivityTimeline>, ClaudeToolsError> {
            ...
            if path.extension().and_then(|s| s.to_str()) == Some("json") {
                ...
                if let Ok(metadata) = fs::metadata(&path) {
                    ...
                    if let Ok(cache_metadata) = self.load_cache_metadata(&path) {
                        ...
                        if stats.oldest_cache == DateTime::<Utc>::MIN_UTC
                            || cache_metadata.created_at < stats.oldest_cache
                        {
