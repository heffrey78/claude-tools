...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Messages: {}", conversations.iter().map(|c| c.messages.len()).sum::<usize>());
    ...
    if let Some((file_path, project_name, file_size)) = largest_file {
        ...
        println!("   Messages (traditional): {}", full_conversation.messages.len());
        println!("   Messages (streaming): {}", streaming_conversation.messages.len());
        ...
        if metadata.line_count > 10 {
            ...
            println!("\n🎯 Seeking Performance:");
            println!("   Seek to middle entry: {:.3}ms", seek_time.as_secs_f64() * 1000.0);
            println!("   Target: <1ms ✅");
        }
        ...
        if path.is_dir() {
            ...
            for file_entry in std::fs::read_dir(&path)? {
                ...
                if file_path.extension().and_then(|s| s.to_str()) == Some("jsonl") {
                    ...
                    if largest.as_ref().map_or(true, |(_, _, size)| file_size > *size) {
